...
class InstallManager(object):
    ...
    def get_selected_engine_from_parameters(self, parameters):
        """
        ...
        if KnownParameters.SELECTED_ENGINE.value in parameters:
            ...
            if engine_name:
                return self.core.feed.get_product(engine_name)
        return None
    ...
    def install(self):
        ...
        logging.info('products to install: {0}'.format(self.products.to_dict()))
        ...
        for product in self.products:
            try:
                ...
                if product.install(parameters):
                    #logging.info('update current yaml')
                    self.core.set_product_installed(product, parameters)
            except Exception as ex:
                raise Exception("Can't install product {0}, Exception".format(product)) from ex
