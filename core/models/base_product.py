...
class BaseProduct(object):
    ...
    def merge(self, **kwargs):
        ...
        self.installer = Installer(self, self.install_command, self.upgrade_command, self.uninstall_command, self.find_installed_command)
    ...
    def find_installed(self)-> InstalledProductInfo:
        ...
        """
        return self.installer.find_installed()
    ...
    def set_log_path(self, action: str):
        ...
        self.envs['%{0}_LOG_PATH%'.format(action.upper())] = \
            LogManager.get_instance(self.core).get_log_path(self, action)

        self.envs['%{0}_LOG_DIR%'.format(action.upper())] = \
            LogManager.get_instance(self.core).get_log_dir(self)
    ...
    def unset_log_path(self, action):
        ...
        :param action: name of current action: 'install', 'uninstall', etc.
        """
        del self.envs['%{0}_LOG_PATH%'.format(action.upper())]
